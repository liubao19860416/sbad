...
public interface BaseService<T> {
     ...
     */
    Long deleteEntityCache(T record);
    
    /**
     * 根据实体对象T,从缓存中清除对应的数据集合
     */
    Long deleteEntityListCache(T record);

    /**
     * 从缓存中清除所有数据实体集合
     */
    Long deleteEntityAllCache();
     ...
     * 查询指定交易号的记录列表
     */
    List<T> selectByTransactionId(String transactionId);

    /**
     * 根据codeList更新UploadStatus状态标记
     */
    int updateUploadStatusByCodeList(List<String> codeList);

    /**
     * 更新对应的custcd数据状态
     */
    int updateUploadStatusByCustcdList(List<String> custcdList, UploadStatus uploadStatus);
